...
if($state == 1)
{
	$action = "Mod is banning from games user ".$steamid;
}
else
{
	$action = "Mod is unbanning from games user ".$steamid;
}
...
$stmt = $db->prepare('UPDATE users SET gameban=:gameban WHERE steamid=:steamid');
		$stmt->bindValue(":gameban", $state);
		$stmt->bindValue(":steamid", $steamid);
		$stmt->execute();
echo jsonSuccess(1);
?>