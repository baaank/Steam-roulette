$(document).ready(function () {
	...
	function loadItems()
	{
		$.getJSON('/php/history.php', function(jsonObj) {
			var even = 0;
			var games = jsonObj.data;
			for(var i = 0; i < games.length; i++)
			{
				var button = "";
				if(games[i].seed != "in use")
					button = '<td><button class="btn btn-info" id="showRolls" date="'+games[i].date+'" rolls="'+games[i].rolls+'" seed="'+games[i].seed+'">Show rolls</button></td>'
				if(even==0)
				{
					$('#gamesTable').append('<tr><td>'+games[i].date+'</td><td>'+games[i].seed+'</td><td>'+games[i].hash+'</td><td>'+games[i].rolls+'</td>'+button+'</tr>');	
					even=1;
				}
				else if(even==1)
				{
					$('#gamesTable').append('<tr class="active"><td>'+games[i].date+'</td><td>'+games[i].seed+'</td><td>'+games[i].hash+'</td><td>'+games[i].rolls+'</td>'+button+'</tr>');	
					even = 0;
				}
	...
	$(document).on("click", "#showRolls", function () {
		$("#showRoll").modal('show');
		$('#rolls').html('');
		generateRolls(parseInt($(this).attr("rolls")), $(this).attr("seed"), parseInt(0), 0);
		$('#date').html($(this).attr("date"));
		
	});
	function generateRolls(rolls, seed, counter, roundnumber){
			
			if(roundnumber%10 == 0 || counter == 0)
			{
				if(counter == 0)
					$('#rolls').append('<div class="col-md-2">1 - 10</div>')
				else
				{
					if(roundnumber==parseInt(rolls) || roundnumber+10>parseInt(rolls))
						$('#rolls').append('<div class="col-md-2">'+parseInt(counter/10)+1+' - '+parseInt(rolls)+'</div>')
					else
						$('#rolls').append('<div class="col-md-2">'+parseInt(counter/10)+1+' - '+parseInt(parseInt(counter/10+1)*10)+'</div>')
				}
				
			}
			roundnumber += 1;
			var hash =SHA256(seed+roundnumber.toString());
			var num = 0;
			for(var i = 0; i < 32; i++)
			{
			...
			{
				$('#rolls').append('<div class="col-md-1"><div id="squareRed">'+number+'</div></div>');
			}
			else if(number == 0 || number == 14 || number == 13)
			{
				$('#rolls').append('<div class="col-md-1"><div id="squareGreen">'+number+'</div></div>');
			}
			else
			{
				$('#rolls').append('<div class="col-md-1"><div id="squareBlack">'+number+'</div></div>');
			}
			
			parseInt(counter++);
			if(counter<rolls)
				generateRolls(rolls, seed, counter, roundnumber);
		}
	$(document).on("click", "#test-button", function () {
		$('#test-result').html('');
		var sha = document.getElementById("hash-input").value;
		var seed = document.getElementById("seed-input").value;
		var round = document.getElementById("round-input").value;
		
		if(SHA256(seed)!=sha)
		{
			$('#test-result').html('Round hash does not match with seed');
			return
		}
		var hash =SHA256(seed+round);
		var num = 0;
		...
		if(number == 32 || number == 19 || number == 21 || number == 25 || number == 34 || number == 27 || number == 36 || number == 30 || number == 23 || number == 5 || number == 16 || number == 1 || number == 9 || number == 18 || 	number == 7 || number == 12 || number == 3)
		{
			$('#result').html('<div class="col-md-5"></div><div class="col-md-2"><div id="squareRed1">'+number+'</div></div><div class="col-md-5"></div>');
		}
		else if(number == 0 || number == 14 || number == 13)
		{
			$('#result').html('<div class="col-md-5"></div><div class="col-md-2"><div id="squareGreen1">'+number+'</div></div><div class="col-md-5"></div>');
		}
		else
		{
			$('#result').html('<div class="col-md-5"></div><div class="col-md-2"><div id="squareBlack1">'+number+'</div></div><div class="col-md-5"></div>');
		}
		//$('#test-result').html(round)
	});
